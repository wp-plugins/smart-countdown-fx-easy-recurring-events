...
define( 'SCD_MINIMUM_REQUIRED_WP_VERSION', '3.6' );
	...
	public function widget( $args, $instance ) {
		...
		if ( is_customize_preview() ) {
			...
			if( !empty( $customized ) ) {
				...
				// check if current widget is being customized
				$instance['customize_preview'] = in_array($instance['id'], $scd_widgets_ids);
...
<p>
	<label for="<?php echo $this->get_field_id( 'layout_preset' ); ?>"><?php _e( 'Widget layout preset:', 'smart-countdown' ); ?></label>
		...
		echo SmartCountdown_Helper::selectInput( $this->get_field_id( 'layout_preset' ), $this->get_field_name( 'layout_preset' ), $layout_preset, array (
				'type' => 'filelist',
				'extension' => 'xml',
				'folder' => dirname( __FILE__ ) . '/includes/layouts' 
		) );
		?></p>
<p>
