...
Scheduled events will not have any duration set, it is presumed that they do no have end dates (other event import plugins which are in developement right now will support event durations when set in plugins or services the events are pulled from). Nevertheless you can simulate recurring event duration using shortcode "mode" attribute. E.g. mode="countdown:-1,countup:3600" will show countdown to the next scheduled event and after event starts it will show "In progress" (or whatever title you set for count up mode) during 1 hour. Then the counter will automatically switch to next event countdown.
