...
class SmartCountdownEasyRecurring_Helper {
	...
	private static function addEasyRecurrenceChain( &$imported, $unit, $duration, $countdown_to_end, $hour, $minute, $date = '', $month = '', $weekdays = array(), $base_date = null ) {
		...
		if( !$countdown_to_end || $duration == 0 ) {
			// event in countdown-to-end mode we ignore event end if duration is zero
			// (otherwise duplicate start/end events will be created, which is nonsense)
			
			// normal event duration mode - show count up from event start during the event
			$imported[] = array(
			...
			// countdown-to-end mode - show countdown to event end during the event
			$prev_end = clone( $prev_date );
			$prev_end->modify(  '+' . $duration . ' second' );
			$current_end = clone( $current_date );
			$current_end->modify(  '+' . $duration . ' second' );
			$next_end = clone( $next_date );
			$next_end->modify(  '+' . $duration . ' second' );
			$postnext_end = clone( $postnext_date );
			$postnext_end->modify(  '+' . $duration . ' second' );
			
			// we have to force all event durations to zero in "countdown-to-end" mode because we
			// can NEVER show a real count up: title "up" in widget configuration will be reused as
			// title for countdown to event end and will be absurd if displayed with an incrementing
			// counter!
			$imported[] = array(
					'deadline' => self::dateToUTC( $prev_date ),
					'title' => '',
			...
			$imported[] = array(
					'deadline' => self::dateToUTC( $prev_end ),
					'title' => '',
					'duration' => 0,
					'is_countdown_to_end' => 1 // countdown-to-end flag
			);
			$imported[] = array(
					'deadline' => self::dateToUTC( $current_date ),
					'title' => '',
			...
			$imported[] = array(
					'deadline' => self::dateToUTC( $current_end ),
					'title' => '',
					'duration' => 0,
					'is_countdown_to_end' => 1 // countdown-to-end flag
			);
			...
			$imported[] = array(
					'deadline' => self::dateToUTC( $next_end ),
					'title' => '',
					'duration' => 0,
					'is_countdown_to_end' => 1 // countdown-to-end flag
			);
			...
			$imported[] = array(
					'deadline' => self::dateToUTC( $postnext_end ),
					'title' => '',
					'duration' => 0,
					'is_countdown_to_end' => 1 // countdown-to-end flag
			);
	...
	private static function dateToUTC( $date ) {
		...
		$result->modify( ( self::$utcOffset < 0 ? '+' : '-' ) . abs( self::$utcOffset ) . ' second' );
