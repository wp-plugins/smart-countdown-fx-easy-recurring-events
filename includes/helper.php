...
class SmartCountdownEasyRecurring_Helper {
	...
	public static function getEvents( $instance, $configs ) {
		...
		foreach( $configs as $config ) {
			...
			if( $config['pattern'] == 'daily' ) {
				self::addEasyRecurrenceChain( $imported, 'day', $duration, $config['hour'], $config['minute'] );
			} elseif( $config['pattern'] == 'weekly' ) {
				self::addEasyRecurrenceChain( $imported, 'weekday', $duration, $config['hour'], $config['minute'], null, null, $config['weekdays'] );
			} elseif( $config['pattern'] == 'monthly' ) {
				self::addEasyRecurrenceChain( $imported, 'month', $duration, $config['hour'], $config['minute'], $config['date'] );
			} else {
				// yearly
				self::addEasyRecurrenceChain( $imported, 'year', $duration, $config['hour'], $config['minute'], $config['date'], $config['month'] );
			}
	...
	private static function addEasyRecurrenceChain( &$imported, $unit, $duration, $hour, $minute, $date = '', $month = '', $weekdays = array(), $base_date = null ) {
		...
		try {
			if( $unit == 'day' || $unit == 'week' ) {
				...
				foreach( $base_dates as $anchor ) {
					self::addEasyRecurrenceChain( $imported, 'week', $duration, $hour, $minute, null, null, null, $anchor );
				}
				return;
			}
		} catch( Exception $e ) {
